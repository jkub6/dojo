...
**Dojo** is a professional-grade static site and document generator powered by
[Ninja](https://ninja-build.org/) and [Pandoc](https://pandoc.org/). It combines the flexibility of
Pandoc's document conversion with the extreme speed of Ninja's incremental build system.
...
Traditional static site generators (SSGs) often struggle with complex document pipelines (like
generating high-quality PDFs from HTML slides) or deep asset dependency trees. Dojo treats your
website as a build graph, ensuring that changing a single CSS variable or a nested image only
rebuilds what is strictly necessary, while providing the full power of Pandoc for document
transformation.
...
3. **Run Ninja**: Dojo generates a `_build/build.ninja` file. Run ninja to finalize the build:
